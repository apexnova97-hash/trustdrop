  ...
  if (done) {
    return (
      <div style={{ minHeight: '100vh', background: '#050810', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, fontFamily: "'Inter', sans-serif" }}>
        ...
        <div style={{ background: '#0a0e1a', border: '1px solid #141c2e', borderRadius: 20, padding: '48px 40px', maxWidth: 420, width: '100%', textAlign: 'center' }}>
          <div style={{ fontSize: 40, marginBottom: 20 }}>📧</div>
          <h2 style={{ fontSize: 22, fontWeight: 700, color: 'white', marginBottom: 10 }}>Check your email</h2>
          <p style={{ color: '#3a4a6b', fontSize: 15, lineHeight: 1.6 }}>
            We sent a confirmation link to <span style={{ color: '#60a5fa' }}>{email}</span>
          </p>
        </div>
      </div>
    )
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#050810', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ background: '#0a0e1a', border: '1px solid #141c2e', borderRadius: 20, padding: '40px 36px', maxWidth: 420, width: '100%', position: 'relative' }}>
        ...
        <h1 style={{ fontSize: 22, fontWeight: 700, color: 'white', marginBottom: 6 }}>Create your account</h1>
        <p style={{ color: '#3a4a6b', fontSize: 14, marginBottom: 28 }}>Start collecting reviews in minutes. Free for 14 days.</p>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <input type="text" value={businessName} onChange={e => setBusinessName(e.target.value)} placeholder="My Business" className="field" />
        </div>
        ...
        <button className="submit" onClick={handleSignup} disabled={!email || !password || !businessName || loading}>
          {loading ? 'Creating account...' : 'Create account'}
        </button>

        <p style={{ textAlign: 'center', fontSize: 14, color: '#3a4a6b', marginTop: 24 }}>
          Already have an account?{' '}
          <a href="/login" style={{ color: '#60a5fa', textDecoration: 'none', fontWeight: 500 }}>Log in</a>
        </p>
      </div>
    </div>
  )
